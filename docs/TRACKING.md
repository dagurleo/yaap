...
For npm applications, use the typed [`@yaap/client` package](../packages/client/README.md). Its `init({ siteId })` API uses the same tracker as `/script.js`, with safe server-side imports and `destroy()` for lifecycle cleanup. The npm server default is `https://yaap.dagurleo.workers.dev`; self-hosted installs pass `host: "https://analytics.example.com"`, and local development uses `host: "http://localhost:8790"`. Script tags always send events to the origin they load from. The initial npm release is prepared but not published. Existing snippets and browser APIs below continue to work.

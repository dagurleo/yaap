...
- The immediate-upgrade slice of Stage 4 is complete in code: owners can preview and confirm a higher tier from Billing. The confirmation discloses retained usage, remaining capacity, a locally calculated pre-tax estimate, Polar's authoritative renewal amount/date and pending-change replacement. Execution is revision-bound, durable and idempotent, updates the existing Polar subscription with `proration_behavior=invoice`, preserves the current usage period/counts, and grants capacity only after authoritative reconciliation. Failed payment leaves the old entitlement intact; ambiguous results remain recoverable. Scheduled downgrades and the remaining renewal/cancellation lifecycle are still outstanding.
- The first Stage 5 Billing experience slice is complete: owners can open `/app/billing` from the Websites page or account menu, review service state, plan, period, pooled usage, processing reservations and per-site usage, choose an initial plan, resume checkout, upgrade an active subscription, and open the hosted customer portal. Checkout and portal returns now land on Billing; pending activation performs bounded authoritative reconciliation and never trusts the return URL as proof of payment. The page has responsive plan selection plus explicit self-hosted, cancelled, pending, delayed, empty and error states. Usage buffer and trial-grace notices are visible in-app and delivered through the durable email job ledger. Lower tiers remain disabled until scheduled downgrades are implemented. Viewer collection-pause messaging and browser verification against a hosted sandbox remain Stage 5 work.

...
4. Set `DATABASE_URL` as a **build/CI secret** pointing directly at the same production database, using the TLS settings from PlanetScale. Use port `5432` and `sslmode=verify-full`, as in [PlanetScale's Node.js guide](https://planetscale.com/docs/postgres/tutorials/planetscale-postgres-node). If the copied URL also includes `sslrootcert=system`, the deployment and standalone migration scripts translate it to Node's trusted CA certificates with certificate and hostname verification enabled. Keep the existing `BETTER_AUTH_SECRET` and `BOOTSTRAP_SECRET` as Worker runtime secrets. Hyperdrive supplies the runtime connection string; do not put database credentials in Wrangler vars or commit them.
